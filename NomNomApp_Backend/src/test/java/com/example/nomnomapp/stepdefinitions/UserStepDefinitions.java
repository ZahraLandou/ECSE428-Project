...
public class UserStepDefinitions {
    ...
    public void i_delete_the_user_with_id(int userId) {
        try {
            ...
            userDatabase.remove(userId);  // Remove AFTER verification

            // Ensure repository delete is called with the correct user
            if (userToDelete.isPresent()) {
                verify(userRepository).delete(userToDelete.get());
            }
